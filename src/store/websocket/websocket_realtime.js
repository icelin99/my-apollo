    ...
    this.worker.onmessage = (event) => {
      ...
      switch (message.type) {
        ...
        case 'SimWorldUpdate':
          ...
          //-------------------2022.1.21
          if((STORE.moduleDelay.get('chassis').delay > 0) &&
              (STORE.moduleDelay.get('chassis').delay < 2000)) {
            STORE.world = message;
            console.log('STORE.world = window.store', STORE.world);
            const obj = window.clonestore(STORE);
            obj.world = message;
            worldData.push(message);
            console.log('worldData',worldData);
            console.log('obj', obj);
            STORES.push(obj);
            // window.stores.push(obj);
            console.log('STORES = window.stores', STORES);
          }
          if(STORE.moduleDelay && (STORE.moduleDelay.get('chassis').delay > 2000) && download_count === 0) {
            //----------------2022.1.21
            //DownloadFile(worldData, 'worldData.txt');
            download_count = 1;
...
console.log('websocketdata',worldData);
export {worldData};