...
window.clonestore = function clonestore(store) {
  const obj = [];
  obj.timestamp = window.clone(store.timestamp);
  obj.isInitialized = window.clone(store.isInitialized);
  obj.hmi = window.clone(store.hmi);
  obj.planningData = window.clone(store.planningData);
  obj.controlData = window.clone(store.controlData);
  obj.latency = window.clone(store.latency);
  obj.playback = window.clone(store.playback);
  obj.trafficSignal = window.clone(store.trafficSignal);
  obj.meters = window.clone(store.meters);
  obj.options = window.clone(store.options);
  obj.routeEditingManager = window.clone(store.routeEditingManager);
  obj.geolocation = window.clone(store.geolocation);
  obj.moduleDelay = window.clone(store.moduleDelay);
  obj.cameraData = window.clone(store.cameraData);
  obj.storyTellers = window.clone(store.storyTellers);
  obj.teleop = window.clone(store.teleop);
  obj.dimension = window.clone(store.dimension);
  obj.newDisengagementReminder = window.clone(store.newDisengagementReminder);
  obj.offlineViewErrorMsg = window.clone(store.offlineViewErrorMsg);
  return obj;
};
...
class DreamviewStore {
    ...
    handleOptionToggle(option) {
      ...
      if (option === 'showRouteEditingBar') {
        ...
        this.routeEditingManager.enablesRouteEditing();
      }
    }
...
const STORE = window.store =  new DreamviewStore();
